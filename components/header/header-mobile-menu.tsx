...
export function HeaderMobileMenu({ 
  ...
  return (
    <>
      {/* Mobile Menu Button */}
      <button 
        className="md:hidden p-2 rounded-lg hover:bg-white/10 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
        onClick={onToggle}
      ...
      >
        {isOpen ? (
          <CloseIcon className={`w-6 h-6 ${isScrolled ? "text-[#1F1F1F]" : "text-white"}`} />
        ) : (
          <MenuIcon className={`w-6 h-6 ${isScrolled ? "text-[#1F1F1F]" : "text-white"}`} />
        )}
      </button>
