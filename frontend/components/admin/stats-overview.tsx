    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <StatCard
          ...
          value={(stats || statsData).totalSubscribers.toLocaleString()}
          change={`+${(stats || statsData).newSubscribers} νέοι αυτόν τον μήνα`}
          changeType={(stats || statsData).newSubscribers > 0 ? "positive" : "neutral"}
          icon={Users}
          color="#3B82F6"
        />
        <StatCard
          title="Δημοσιευμένα Άρθρα"
          value={(stats || statsData).activePosts}
          change={`${(stats || statsData).activePosts > 0 ? 'Ενεργά' : 'Δεν υπάρχουν άρθρα'}`}
          changeType={(stats || statsData).activePosts > 0 ? "positive" : "neutral"}
          icon={BookOpen}
          color="#10B981"
        />
        <StatCard
          title="Νέοι Εγγεγραμμένοι"
          value={(stats || statsData).newSubscribers}
        ...
        <StatCard
          ...
          value={(stats || statsData).totalViews.toLocaleString()}
        ...
        <StatCard
          ...
          value={(stats || statsData).totalLikes.toLocaleString()}
          change="όλων των άρθρων"
          changeType="positive"
          icon={Heart}
          color="#EF4444"
        ...
        <StatCard
          ...
          value={(stats || statsData).activePosts > 0 ? Math.round((stats || statsData).totalViews / (stats || statsData).activePosts) : 0}
          change="ανά άρθρο"
          changeType="positive"
          icon={TrendingUp}
          color="#06B6D4"
